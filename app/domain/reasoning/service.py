    ...
    try:
        ...
        if use_simple_mode:
            sys_prompt = (
                ...
                "\nAT THE END:\n---\n🤖 **PERSEUS VERDICT**\n"
                "\n\n⚠️ HALLUCINATION RULE: Only cite numbers that appear verbatim in the data above. If backtest stats, win rate, Sharpe ratio, or news events are NOT in the injected data, write \"[data not available]\" — never invent, estimate, or recall from training. A fabricated win rate is a financial liability."
        ...
        if total_tokens > 0 and user_id != "default":
            try:
                import os
                from supabase import create_client
                from datetime import date
                ...
                if existing.data:
                    ...
                    sb.table("token_usage") \
                        .update({"tokens_used": row["tokens_used"] + total_tokens}) \
                        .eq("id", row["id"]) \
                        .execute()
                else:
                    sb.table("token_usage") \
                        .insert({"user_id": user_id, "date": today, "tokens_used": total_tokens, "symbol": symbol}) \
                        .execute()
