...
async def razorpay_webhook(request: Request):
    ...
    # Idempotency — ignore already-processed webhook IDs
    webhook_id = payload.get("id", "")
    if webhook_id:
        try:
            sb = _sb()
            existing = sb.table("processed_webhooks") \
                .select("id") \
                .eq("webhook_id", webhook_id) \
                .execute()
            if existing.data:
                log.info(f"[rzp webhook] duplicate {webhook_id} — skipping")
                return {"status": "duplicate", "webhook_id": webhook_id}
            sb.table("processed_webhooks") \
                .insert({"webhook_id": webhook_id, "event": event}) \
                .execute()
        except Exception as _idem_e:
            log.warning(f"[rzp webhook] idempotency check failed: {_idem_e}")
